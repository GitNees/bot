...
class InteractPostLikers(Plugin):
    """Handles the functionality of interacting with someones post likers"""
    ...
    def run(self, device, device_id, args, enabled, storage, sessions, plugin):
        ...
        sources = [source for source in (args.post_likers)]
        shuffle(sources)
    ...
    def handle_profile(
        ...
        ProfileView(device)._getFistPostView(posts_list_view).click()
        random_sleep()
